'''
可以完整並長時間運作的104爬蟲，包含爬search list的Job ID及其他頁面資訊，再依照爬到的ID進去職缺頁面爬其他資訊並存成csv及json
主要使用selenium方式進行模擬、以css selector為主，部分資訊使用xpath方式
其他還有包含斷點存檔設計、進行頁數紀錄，再執行會依照checkpoint的紀錄開始、瀏覽器重整設計
'''
